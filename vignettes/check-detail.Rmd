...
- _choices does not contain column 'name'_ (critical)
- _choices does not contain column 'list_name'_ (critical)
...
- _illegal dependent.var.type_ (critical)
- _illegal independent.var.type_ (critical)
- _non empty row without dependent.var - skipping this row _ (critical)
- _logical conditions disrespected (i.e. can't have group_difference without indepdendent.var)_
...
### Questionnaire vs. Choices

- _could not check questionnaire against choices (critical)

### Choices vs. Data

- _SMchoices are not list_name.choice_name_ (critical)

### Data vs. Sampling Frame

- _sample bigger than population_ (critical)
- _strata names not in sampling frame_
